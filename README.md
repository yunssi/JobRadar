...
한국도로공사서비스·우체국시설관리단 채용 API·서울물재생시설공단 서버는 각각 필요한 TLS 중간
인증서를 전송하지 않으므로 해당 출처에만 인증기관의 공식 중간 인증서 파일을 추가합니다.
`tls_ca_file`은 저장소 안의 유효한 인증서와 HTTPS URL에만 사용할 수 있으며, 시스템 신뢰
저장소·인증서 검증·호스트명 검증은 그대로 유지됩니다.
...
클라우드 IP에서 공식 게시판 접속을 막는 출처는 공식 홈페이지 링크를 유지하면서 회사의 사람인·잡코리아
채용 페이지 중 `진행중 공고` 섹션만 전용 `document_adapter`로 읽습니다. 공식 채용 화면이 별도
HRDMS 포털을 사용하는 출처는 같은 포털의 진행 중 공고 API를 XHR 방식으로 읽고, 사용자 링크는
공식 지원 화면으로 연결합니다.
...
- 우체국시설관리단 채용 API의 인증서 오류도 같은 원칙으로 Sectigo DV R36 중간 인증서와 발급자를
  확인합니다.
